...
function Proccess(state, controls){
    ...
    this.ctrlKeyPress = async (command, key) => {
        triggerProccessEvent(WM.CONTROL_KEY, VK[key]);
    };

    this.keyPress = async (command, key) => {
        triggerProccessEvent(WM.KEY, VK[key]);
    };

    this.altKeyPress = async (command, key) => {
        triggerProccessEvent(WM.ALT_KEY, VK[key]);
    };

    this.shiftKeyPress = async (command, key) => {
        triggerProccessEvent(WM.SHIFT_KEY, VK[key]);
    };
